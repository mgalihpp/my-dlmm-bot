...
const JUPITER_API_KEY =
	"jup_c5bbdfe316b65f3db508f78f9ece2508b7d7e3e7704f4d04e282b273a60b14c9";
...
function toPubkey(val: unknown): PublicKey {
	...
	if (typeof obj.toBase58 === "function")
		return new PublicKey((obj.toBase58 as () => string)());
	...
	if (obj._bn && typeof obj._bn === "object" && "toArray" in obj._bn) {
		return new PublicKey(
			Buffer.from((obj._bn as { toArray: () => number[] }).toArray()),
		);
	}
	throw new Error(
		`Cannot convert to PublicKey: type=${typeof val} ctor=${(obj.constructor as { name?: string })?.name}`,
	);
}
...
const jupiterUltraSwapOnce = (
	...
	Effect.gen(function* () {
		...
		const result = yield* HttpClientRequest.post(
			`${JUPITER_API_URL}/swap/v2/execute`,
		).pipe(
...
const swapTokensToOutput = (
	...
	Effect.gen(function* () {
		...
		for (const { mint, amount } of inputs) {
			...
			lastSig = yield* jupiterUltraSwap(
				client,
				keypair,
				mint,
				outputMint,
		...
		swapExactIn: (inputMint, outputMint, amount, slippageBps) =>
			withSigner("swapExactIn", (connection, keypair) =>
				Effect.gen(function* () {
					...
					const signature = yield* jupiterUltraSwap(
						client,
						keypair,
						inPk,
						outPk,
					...
					return {
						signature,
						received: received.ltn(0) ? new BN(0) : received,
						outputMint,
					};
				}),
			),
