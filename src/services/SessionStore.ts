...
export interface SessionStoreService {
	...
	readonly takeInput: (
		chatId: string | number,
	) => Effect.Effect<Option.Option<InputSession>>;
	...
	readonly resolveAction: (
		id: string,
	) => Effect.Effect<Option.Option<ActionEntry>>;
...
const make = Effect.gen(function* () {
	const inputs = yield* Ref.make(
		HashMap.empty<string, Expiring<InputSession>>(),
	);
	const actions = yield* Ref.make(
		HashMap.empty<string, Expiring<ActionEntry>>(),
	);
	const wizards = yield* Ref.make(
		HashMap.empty<string, Expiring<WizardState>>(),
	);
	...
	const service: SessionStoreService = {
		...
		registerAction: (poolAddress, positionPubkey) =>
			Effect.gen(function* () {
				...
				yield* Ref.update(
					actions,
					HashMap.set(id, {
						value: { poolAddress, positionPubkey },
		...
		createWizard: (state) =>
			Effect.gen(function* () {
				...
				yield* Ref.update(
					wizards,
					HashMap.set(id, { value: state, expiresAt: now + WIZARD_TTL_MS }),
				);
				return id;
			}),
