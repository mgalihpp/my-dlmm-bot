...
export const ClosedPnlCard = forwardRef<HTMLDivElement, ClosedPnlCardProps>(
	function ClosedPnlCard({ pool, currency, theme }, ref) {
		...
		const depositStr = fmtClosed(
			pool.totalDeposit,
			pool.totalDepositSol ?? null,
			currency,
		);
		const withdrawStr = fmtClosed(
			pool.totalWithdrawal,
			pool.totalWithdrawalSol ?? null,
			currency,
		);
		const feesStr = fmtClosed(
			pool.totalFee,
			pool.totalFeeSol ?? null,
			currency,
		);
		const pnlStr =
			pnlNumeric >= 0
		...
		const closedLabel = pool.lastClosedAt ? timeAgo(pool.lastClosedAt) : "-";
		const binStepLabel = pool.binStep != null ? String(pool.binStep) : "-";
