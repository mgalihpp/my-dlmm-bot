...
export function PnlShareShell({
	...
	const [cardSize, setCardSize] = useState({ width: 720, height: 420 });
	...
	const updateScale = useCallback(() => {
		...
		const available = vp.clientWidth - padX - 8;
		const w = card.offsetWidth || card.scrollWidth || 720;
		...
		const next = available / w;
		const clamped = next >= 0.99 ? 1 : Math.max(0.26, Math.min(1, next));
	...
	const scalerStyle: React.CSSProperties = isScaled
		? {
				transform: `scale(${scale})`,
				transformOrigin: "top center",
				width: cardSize.width,
				marginBottom: `-${cardSize.height * (1 - scale)}px`,
				marginLeft: `-${(cardSize.width * (1 - scale)) / 2}px`,
				marginRight: `-${(cardSize.width * (1 - scale)) / 2}px`,
				flexShrink: 0,
			}
		: { flexShrink: 0 };
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="flex max-h-[96dvh] w-[calc(100vw-16px)] max-w-[calc(100vw-16px)] flex-col gap-0 overflow-hidden border-[#222] bg-black p-0 sm:max-h-[92vh] sm:w-full sm:max-w-[1100px]">
				...
				<div className="flex min-h-0 flex-1 flex-col overflow-hidden lg:flex-row">
					...
					<div className="flex max-h-[42dvh] w-full flex-col overflow-hidden border-t border-[#222] bg-[#111113] sm:max-h-[50dvh] lg:max-h-none lg:w-[320px] lg:border-t-0 lg:border-l">
									...
									<div className="flex flex-wrap gap-1.5">
										{TEXTURES.map((t) => (
											<button
												...
												className={`relative flex size-8 items-center justify-center overflow-hidden rounded-md border text-[9px] transition ${settings.textureId === t.id ? "border-white bg-white/10 text-white" : "border-white/10 bg-[#1a1a1e] text-white/50 hover:border-white/20 hover:text-white/80"}`}
												title={t.label}
											>
												{t.css ? (
													<span
						...
						<div className="flex shrink-0 gap-2 border-t border-[#222] bg-[#111113] p-4 pt-3 sm:p-5 sm:pt-3 lg:p-5">
							<Button
								variant="outline"
								onClick={handleCopy}
								disabled={exporting}
