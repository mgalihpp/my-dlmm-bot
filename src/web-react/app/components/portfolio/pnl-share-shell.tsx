...
export type PnlShareShellProps = {
	open: boolean;
	onOpenChange: (v: boolean) => void;
	title?: string;
	description?: string;
	filename: string;
	renderCard: (theme: CardTheme, ref: RefObject<HTMLDivElement | null>) => ReactNode;
};
...
export function PnlShareShell({
	...
	onOpenChange,
	title = "Share PnL",
	description = "Export PnL card as image",
	filename,
	renderCard,
}: PnlShareShellProps) {
	...
	const handleDownload = async () => {
		...
		setExporting(true);
		try {
			// html-to-image must be dynamically imported client-side to avoid SSR
			...
			<DialogContent className="flex max-h-[92vh] max-w-[1100px] flex-col gap-0 overflow-hidden border-[#222] bg-black p-0 sm:max-w-[1100px]">
				...
				<div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
					<div className="flex flex-1 items-center justify-center overflow-auto bg-[#050505] p-4 lg:p-6">
						{renderCard(theme, cardRef as RefObject<HTMLDivElement | null>)}
					</div>
							...
							<div className="grid grid-cols-4 gap-2">
								{BACKGROUNDS.map((b) => (
									<button
										...
										style={{
											background: b.value === "transparent" ? "#0a0a0a" : b.value,
							...
							{customImage ? (
								<div className="space-y-2 rounded-md border border-[#222] p-3">
									...
									<div className="flex items-center justify-between text-xs text-zinc-400">
										...
										<select
											value={textMode}
											onChange={(e) => setTextMode(e.target.value as "light" | "dark")}
											className="rounded border border-[#222] bg-black px-1 py-0.5 text-xs"
										>
