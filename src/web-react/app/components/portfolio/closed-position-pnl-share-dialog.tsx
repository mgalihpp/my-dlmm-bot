...
export function ClosedPositionPnlShareDialog({
	...
	const handleDownload = async () => {
		...
		try {
			...
			link.download = `pnl-closed-pos-${position.positionAddress.slice(0, 6)}.png`;
			link.href = dataUrl;
			link.click();
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="flex max-h-[92vh] max-w-[1100px] flex-col gap-0 overflow-hidden border-[#222] bg-black p-0 sm:max-w-[1100px]">
				<DialogHeader className="sr-only">
					<DialogTitle>Share Closed Position PnL</DialogTitle>
					<DialogDescription>
						Preview and export your closed position PnL card
					</DialogDescription>
				</DialogHeader>
