...
export function buildDiscoveryFilter(
	cfg: PoolsConfig | undefined,
	_timeframe?: string,
): string {
	const s = cfg ?? {};
	...
	if (s.baseTokenHasHighSupplyConcentration != null)
		filters.push(
			`base_token_has_high_supply_concentration=${s.baseTokenHasHighSupplyConcentration}`,
		);
	if (s.baseTokenHasHighSingleOwnership != null)
		filters.push(
			`base_token_has_high_single_ownership=${s.baseTokenHasHighSingleOwnership}`,
		);
	...
	if (s.minOrganic != null)
		filters.push(`base_token_organic_score>=${s.minOrganic}`);
	if (s.maxOrganic != null)
		filters.push(`base_token_organic_score<=${s.maxOrganic}`);
	...
	if (s.minQuoteOrganic != null)
		filters.push(`quote_token_organic_score>=${s.minQuoteOrganic}`);
	if (s.maxQuoteOrganic != null)
		filters.push(`quote_token_organic_score<=${s.maxQuoteOrganic}`);
	...
	if (s.minFeeActiveTvlRatio != null)
		filters.push(`fee_active_tvl_ratio>=${s.minFeeActiveTvlRatio}`);
	if (s.maxFeeActiveTvlRatio != null)
		filters.push(`fee_active_tvl_ratio<=${s.maxFeeActiveTvlRatio}`);
	...
	if (s.minActivePositions != null)
		filters.push(`active_positions>=${s.minActivePositions}`);
	if (s.maxActivePositions != null)
		filters.push(`active_positions<=${s.maxActivePositions}`);
	if (s.minOpenPositions != null)
		filters.push(`open_positions>=${s.minOpenPositions}`);
	if (s.maxOpenPositions != null)
		filters.push(`open_positions<=${s.maxOpenPositions}`);
	...
	if (s.minUniqueTraders != null)
		filters.push(`unique_traders>=${s.minUniqueTraders}`);
	if (s.maxUniqueTraders != null)
		filters.push(`unique_traders<=${s.maxUniqueTraders}`);
	if (s.minPriceChangePct != null)
		filters.push(`pool_price_change_pct>=${s.minPriceChangePct}`);
	if (s.maxPriceChangePct != null)
		filters.push(`pool_price_change_pct<=${s.maxPriceChangePct}`);
	if (s.minVolumeChangePct != null)
		filters.push(`volume_change_pct>=${s.minVolumeChangePct}`);
	if (s.maxVolumeChangePct != null)
		filters.push(`volume_change_pct<=${s.maxVolumeChangePct}`);
	if (s.priceTrend != null) filters.push(`price_trend=${s.priceTrend}`);
	if (s.solPairOnly === true) {
		filters.push(
			"(token_x=So11111111111111111111111111111111111111112||token_y=So11111111111111111111111111111111111111112)",
		);
	}
	return filters.join("&&");
}
