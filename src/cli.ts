...
const configCmd = Command.make("config", {}, () =>
	...
	Command.withDescription(
		"show the active config and where it was loaded from",
	),
);

const dryRunFlag = Options.boolean("dry-run").pipe(
	Options.withDescription("preview without sending transaction"),
);
...
const positionCreateCmd = Command.make(
	"create",
	{
		...
		strategy: Options.text("strategy").pipe(
			Options.withDescription("strategy: spot, bidask, or curve"),
		),
		xAmount: Options.text("x-amount").pipe(
			Options.withDescription("amount of token X (human, e.g. 0.5)"),
		),
		yAmount: Options.text("y-amount").pipe(
			Options.withDescription("amount of token Y (human, e.g. 0.5)"),
		),
		minBin: Options.integer("min-bin").pipe(
			Options.withDescription("minimum bin ID (absolute)"),
			Options.optional,
		),
		maxBin: Options.integer("max-bin").pipe(
			Options.withDescription("maximum bin ID (absolute)"),
			Options.optional,
		),
		...
		maxPct: Options.float("max-pct").pipe(
			Options.withDescription("max % vs current price, e.g. 0"),
			Options.optional,
		),
		atomic: Options.boolean("atomic").pipe(
			Options.withDescription("treat amounts as atomic units, not human"),
		),
		autoFill: Options.boolean("auto-fill").pipe(
			Options.withDescription(
				"auto-fill the missing side from the active-bin ratio",
			),
		),
		singleSided: Options.boolean("single-sided").pipe(
			Options.withDescription("single-sided: deposit only token X (meme)"),
		),
...
const liquidityAddCmd = Command.make(
	"add",
	{
		...
		xAmount: Options.text("x-amount").pipe(
			Options.withDescription("amount of token X to add"),
		),
		yAmount: Options.text("y-amount").pipe(
...
const liquidityRemoveCmd = Command.make(
	"remove",
	{
		poolAddress: Args.text({ name: "poolAddress" }),
		positionPubkey: Args.text({ name: "positionPubkey" }),
		bps: Options.integer("bps").pipe(
			Options.withDescription("basis points to remove (1-10000)"),
		),
...
const claimAction =
	...
	(opts: {
		...
		Effect.gen(function* () {
			...
			yield* Console.log(`  Pool:     ${cyan(opts.poolAddress)}`);
			yield* Console.log(
				`  Position: ${gray(shortAddr(opts.positionPubkey))}\n`,
			);
...
const poolListCmd = Command.make(
	"list",
	{
		...
		category: Options.text("category").pipe(
			Options.withDescription("pool category: trending, new, top"),
			Options.optional,
		),
		limit: Options.integer("limit").pipe(
			Options.withDescription("max pools to show"),
...
const watchAddCmd = Command.make(
	"add",
	{
		...
		label: Options.text("label").pipe(
			Options.withAlias("l"),
			Options.withDescription("friendly label for the wallet"),
			Options.optional,
		),
	},
	(opts) =>
		Effect.gen(function* () {
			...
			const entry = yield* watchlist.add(
				opts.wallet,
				Option.getOrUndefined(opts.label),
			);
...
const watchPositionsCmd = Command.make(
	"positions",
	{ json: jsonFlag },
	(opts) =>
		Effect.gen(function* () {
...
const walletsCmd = Command.make(
	...
	Command.withDescription(
		"query open positions for one or more wallets on-the-fly",
	),
);

const rootCmd = Command.make("vexis", {}, () => Effect.void).pipe(
	Command.withDescription(
		"View your Meteora DLMM portfolio (open & closed positions)",
	),
	Command.withSubcommands([
		openCmd,
		closedCmd,
...
function formatCliCostQuote(quote: PositionCostQuote): string {
	...
	lines.push(
		`  Position rent:       ${quote.positionCost.toFixed(4)} SOL (refundable)`,
	);
	if (quote.positionReallocCost > 0) {
		lines.push(
			`  Position extension:  ${quote.positionReallocCost.toFixed(4)} SOL (refundable)`,
		);
	}
	if (quote.binArraysCount > 0) {
		const perBin = (quote.binArrayCost / quote.binArraysCount).toFixed(4);
		lines.push(
			`  New bin arrays:      ${quote.binArraysCount} × ${perBin} SOL = ${quote.binArrayCost.toFixed(4)} SOL (NON-REFUNDABLE)`,
		);
	}
	if (quote.bitmapExtensionCost > 0) {
		lines.push(
			`  Bitmap extension:    ${quote.bitmapExtensionCost.toFixed(4)} SOL (NON-REFUNDABLE)`,
		);
	}
	lines.push(`  Transactions:        ${quote.transactionCount}`);
	if (quote.nonRefundableCost > 0) {
		lines.push(
			`  ${red(`⚠ ${quote.nonRefundableCost.toFixed(4)} SOL non-refundable`)} — new on-chain accounts for this range`,
		);
	}
	lines.push(
		`  Total:               ${bold(String(quote.totalCost.toFixed(4)))} SOL`,
	);
	return lines.join("\n");
}
