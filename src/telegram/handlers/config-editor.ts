...
const EDITABLE_FIELDS = [
	...
	{
		key: "create.range.type",
		label: "Range Type",
		type: "enum" as const,
		values: ["default", "bin", "pct"],
	},
	{
		key: "create.range.minBin",
		label: "Range Min Bin",
		type: "number" as const,
	},
	{
		key: "create.range.maxBin",
		label: "Range Max Bin",
		type: "number" as const,
	},
	...
	{
		key: "create.amountPresets",
		label: "Amount Presets",
		type: "list" as const,
	},
	{
		key: "create.slippageBps",
		label: "Slippage (bps)",
		type: "number" as const,
	},
	...
	{
		key: "pools.minActiveTvl",
		label: "Min Active TVL",
		type: "number" as const,
	},
	{
		key: "pools.maxActiveTvl",
		label: "Max Active TVL",
		type: "number" as const,
	},
	{ key: "pools.minFee", label: "Min Fee ($)", type: "number" as const },
	{ key: "pools.maxFee", label: "Max Fee ($)", type: "number" as const },
	{
		key: "pools.minFeeActiveTvlRatio",
		label: "Min Fee/TVL",
		type: "number" as const,
	},
	{
		key: "pools.maxFeeActiveTvlRatio",
		label: "Max Fee/TVL",
		type: "number" as const,
	},
	...
	{
		key: "pools.minVolatility",
		label: "Min Volatility",
		type: "number" as const,
	},
	{
		key: "pools.maxVolatility",
		label: "Max Volatility",
		type: "number" as const,
	},
	{
		key: "pools.minPoolPrice",
		label: "Min Pool Price",
		type: "number" as const,
	},
	{
		key: "pools.maxPoolPrice",
		label: "Max Pool Price",
		type: "number" as const,
	},
	{
		key: "pools.minActivePositions",
		label: "Min Active Pos",
		type: "number" as const,
	},
	{
		key: "pools.maxActivePositions",
		label: "Max Active Pos",
		type: "number" as const,
	},
	{
		key: "pools.minOpenPositions",
		label: "Min Open Pos",
		type: "number" as const,
	},
	{
		key: "pools.maxOpenPositions",
		label: "Max Open Pos",
		type: "number" as const,
	},
	...
	{
		key: "pools.minUniqueTraders",
		label: "Min Traders",
		type: "number" as const,
	},
	{
		key: "pools.maxUniqueTraders",
		label: "Max Traders",
		type: "number" as const,
	},
	{
		key: "pools.minPriceChangePct",
		label: "Min Price Chg %",
		type: "number" as const,
	},
	{
		key: "pools.maxPriceChangePct",
		label: "Max Price Chg %",
		type: "number" as const,
	},
	{
		key: "pools.minVolumeChangePct",
		label: "Min Vol Chg %",
		type: "number" as const,
	},
	{
		key: "pools.maxVolumeChangePct",
		label: "Max Vol Chg %",
		type: "number" as const,
	},
	{ key: "pools.priceTrend", label: "Price Trend", type: "string" as const },
	{
		key: "pools.solPairOnly",
		label: "SOL Pair Only",
	...
	{
		key: "pools.minQuoteOrganic",
		label: "Min Q.Organic",
		type: "number" as const,
	},
	{
		key: "pools.maxQuoteOrganic",
		label: "Max Q.Organic",
		type: "number" as const,
	},
	{
		key: "pools.baseTokenHasHighSupplyConcentration",
		label: "High Supply Conc.",
		type: "boolean" as const,
	},
	{
		key: "pools.baseTokenHasHighSingleOwnership",
		label: "High Single Owner",
		type: "boolean" as const,
	},
	{
		key: "pools.minTokenAgeHours",
		label: "Min Token Age (h)",
		type: "number" as const,
	},
	{
		key: "pools.maxTokenAgeHours",
...
function buildConfigText(
	...
	const screeningFields = EDITABLE_FIELDS.filter((f) =>
		f.key.startsWith("pools."),
	);
...
function buildConfigKeyboard(page = 1): InlineKeyboard {
	...
	if (page === 4) {
		return new InlineKeyboard()
			...
			.text(
				"🔄 High Conc",
				"cfg:toggle:pools.baseTokenHasHighSupplyConcentration",
			)
			.row()
			.text("« TVL/Vol", "cfg:page:3")
...
function pageForKey(key: string): number {
	...
	const page4 = new Set([
		...
		"pools.minOrganic",
		"pools.maxOrganic",
		"pools.minQuoteOrganic",
		"pools.maxQuoteOrganic",
		"pools.solPairOnly",
...
export function registerConfigEditor(bot: Bot) {
	...
	bot.callbackQuery(/^cfg:set:(.+)$/, async (ctx) => {
		...
		else if (editable.type === "list")
			typeHint =
				" \\(comma\\-separated numbers, e\\.g\\. 0\\.1, 0\\.25, 0\\.5, 1\\)";
	...
	bot.callbackQuery(/^cfg:reset:(.+)$/, async (ctx) => {
		...
		await ctx.editMessageText(`${tgBold("✅ Reset to default")}\n\n${text}`, {
			...MD,
			reply_markup: buildConfigKeyboard(pageForKey(field)),
		});
	});
	...
	bot.callbackQuery(/^cfg:page:(\d+)$/, async (ctx) => {
		...
		await ctx.editMessageText(text, {
			...MD,
			reply_markup: buildConfigKeyboard(page),
		});
	});
	...
	bot.callbackQuery("cfg:back", async (ctx) => {
		...
		await ctx.editMessageText(text, {
			...MD,
			reply_markup: buildConfigKeyboard(1),
		});
	});
	...
	bot.on("message:text", async (ctx, next) => {
		...
		const text = buildConfigText(getConfigSync(), configPath());
		await ctx.reply(`${tgBold(`✅ ${pending.field} updated`)}\n\n${text}`, {
			...MD,
			reply_markup: buildConfigKeyboard(pending.page),
		});
	});
}
