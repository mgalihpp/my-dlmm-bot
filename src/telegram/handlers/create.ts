		...
		try {
			...
			for (const p of result.pools.slice(0, 10)) {
				...
				lines.push(
					...
					`MC ${tgUsd(p.mcap, idrRate())} \\| TVL ${tgUsd(p.tvl, idrRate())} \\| Vol ${tgUsd(p.volume, idrRate())}`,
					`Fee ${tgUsd(p.fee, idrRate())} \\| Fee/TVL ${escapeMarkdown(`${formatNum(p.feeActiveTvlRatio)}%`)} \\| Holders ${escapeMarkdown(formatNum(p.holders))}`,
				...
				kb.text(
					`${p.baseSymbol}/${p.quoteSymbol} B${p.binStep} ${p.baseFeePct}%`.slice(
						0,
						30,
					),
					`crt:strategy:${wid}`,
		...
		try {
			...
			for (const p of res.pools) {
				...
				try {
					...
					kb.text(
						`${label} B${detail.pool_config.bin_step} ${detail.pool_config.base_fee_pct}%`.slice(
							0,
							30,
						),
						`crt:strategy:${wid}`,
	...
	bot.callbackQuery("crt:from:address", async (ctx) => {
		...
		await ctx.editMessageText(
			[
				...
				escapeMarkdown(
					"Or: https://app.meteora.ag/dlmm/5rCf1DM8LjKTw4YqhnoLcngyZYeNnQqztScTogYHAS6",
				),
			].join("\n"),
			{ ...MD, reply_markup: backToSourceKb() },
		);
		...
		if (pick === "c") {
			...
			setInputSession(chatId, async (text, sessionCtx) => {
				...
				if (Number.isNaN(val) || val <= 0) {
					await sessionCtx.reply(
						"✖ Invalid amount\\. Send SOL budget \\(e\\.g\\. 0\\.5\\):",
						MD,
					);
					return;
				}
			...
			await ctx.editMessageText(
				"✏️ Send *SOL budget* \\(total to deposit, e\\.g\\. 0\\.5\\):",
				MD,
			);
			return;
		}
...
function promptCustomAmount(ctx: Context, wid: string, side: "x" | "y") {
	const chatId = String(ctx.chat?.id ?? ctx.from?.id);
	const word =
		side === "y" ? "Y amount \\(SOL/stable\\)" : "X amount \\(meme token\\)";
	setInputSession(chatId, async (text, sessionCtx) => {
		const val = parseFloat(text);
		if (Number.isNaN(val) || val <= 0) {
 ...
 */
async function executeSwapAndCreate(ctx: Context, wid: string, budget: number) {
	const state = getWizard(wid);
	if (!state) return await expired(ctx);
	const strategy = (state.strategy ?? PRESET.strategy) as StrategyType;
	...
	try {
		...
		const body = (res.signatures ?? [])
			.map((s: string) => tgTxLink(s))
			.join("\n");
		await ctx.editMessageText(`✅ Position created\\!\n${body}`, MD);
	} catch (e) {
...
export async function renderStrategyStep(wid: string): Promise<string> {
	...
	if (state.baseFeePct != null) {
		lines.push(
			`Bin: ${escapeMarkdown(String(state.binStep))} \\| Fee: ${escapeMarkdown(`${state.baseFeePct}%`)}`,
		);
	}
	if (state.tvl != null) {
		lines.push(
			`TVL: ${tgUsd(state.tvl, idrRate())} \\| Holders: ${escapeMarkdown(formatNum(state.holders ?? 0))}`,
		);
	}
	if (state.volume24h != null) {
		lines.push(`Vol 24h: ${tgUsd(state.volume24h, idrRate())}`);
	}
	lines.push(
		"",
...
function formatCostQuote(quote: PositionCostQuote): string {
	...
	if (quote.bitmapExtensionCost > 0) {
		lines.push(
			`⚠️ Bitmap extension: ${escapeMarkdown(quote.bitmapExtensionCost.toFixed(4))} ◎ \\(*non\\-refundable*\\)`,
		);
	}
