...
export function registerWatchlist(bot: Bot) {
	...
	bot.callbackQuery(/^watchadd:label:(.+)$/, async (ctx) => {
		...
		setInputSession(chatId, async (text, sessionCtx) => {
			...
			await sessionCtx.reply(
				`✅ Added ${tgCode(wallet.address)} \\(${escapeMarkdown(wallet.label!)}\\)`,
				MD,
			);
		});
	...
	bot.command("watchpositions", async (ctx) => {
		try {
			...
			if (wallets.length === 0) {
				await ctx.reply(
					"📭 No watched wallets\\. Add one with `/watchadd <wallet>`",
					MD,
				);
				return;
			}
	...
	bot.command("wallets", async (ctx) => {
		try {
			...
			setInputSession(chatId, async (text, sessionCtx) => {
				...
				const loadingMsg = await sessionCtx.reply(
					"⏳ Loading positions\\.\\.\\.",
					MD,
				);
				const results: WalletPositions[] = [];
				...
				const resText = tgMultiWalletPositions(results, await idrRate());
				try {
