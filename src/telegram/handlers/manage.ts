...
export function registerManage(bot: Bot) {
	...
	bot.callbackQuery(/^mng:addliq:strategy:([^:]+):(.+)$/, async (ctx) => {
		...
		const xHandler = async (text: string, sessionCtx: Context) => {
			...
			if (Number.isNaN(val) || val < 0) {
				await sessionCtx.reply(
					`✖ Invalid amount\\. Send a positive number for ${escapeMarkdown(xToken)}:`,
					MD,
				);
			...
			const yHandler = async (text2: string, sessionCtx2: Context) => {
				...
				if (Number.isNaN(val2) || val2 < 0) {
					await sessionCtx2.reply(
						`✖ Invalid amount\\. Send a positive number for ${escapeMarkdown(yToken)}:`,
						MD,
					);
			...
			await sessionCtx.reply(
				`✅ ${escapeMarkdown(xToken)}: ${escapeMarkdown(text)}\n\nNow send ${escapeMarkdown(yToken)} amount \\(or 0 if single\\-sided\\):`,
				MD,
			);
			setInputSession(chatId, yHandler);
		};
		setInputSession(chatId, xHandler);
		await ctx.editMessageText(
			`✏️ Send amount for *${escapeMarkdown(xToken)}* \\(e\\.g\\. 0\\.5\\):`,
			MD,
		);
	});
