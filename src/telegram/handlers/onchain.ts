...
export function registerOnchain(bot: Bot, getRt: () => RuntimeAgent | null) {
	...
	const makeDlmmRunner =
		(fn: (d: typeof dlmm) => Promise<string>) => async (): Promise<string> => {
			return fn(dlmm);
		};

	const makeZapRunner =
		(fn: (z: typeof zap) => Promise<string>) => async (): Promise<string> => {
			return fn(zap);
		};
	...
	bot.command("close", async (ctx) => {
		try {
			await requireKeypair();
			const [poolAddress, positionPubkey] = (ctx.match as string)
				.trim()
				.split(/\s+/);
		...
		try {
			...
			if (!detail) {
				await ctx.editMessageText("Pool not found\\.", {
					...MD,
					reply_markup: new InlineKeyboard().text("⬅️ Back", "close:pools"),
				});
				return;
			}
			const kb = new InlineKeyboard();
			const actionIds = detail.positions.map((p) =>
				registerAction(poolAddr, p),
			);
			detail.positions.forEach((pos, i) => {
				kb.text(
					`#${i + 1}: ${pos.slice(0, 6)}…${pos.slice(-4)}`,
					`close:pos:${actionIds[i]}`,
				).row();
			});
	...
	bot.callbackQuery(/^close:pos:(.+)$/, async (ctx) => {
		...
		if (!pair) {
			await ctx.editMessageText("⌛ Expired\\.", MD);
			return;
		}
	...
	bot.callbackQuery(/^addliq:pool:(.+)$/, async (ctx) => {
		...
		try {
			...
			if (!detail) {
				await ctx.editMessageText("Pool not found\\.", {
					...MD,
					reply_markup: new InlineKeyboard().text("⬅️ Back", "addliq:pools"),
				});
				return;
			}
			const kb = new InlineKeyboard();
	...
	bot.callbackQuery(/^addliq:pos:(.+)$/, async (ctx) => {
		...
		addLiqSessions.set(chatId, {
			poolAddress: pair.poolAddress,
			positionPubkey: pair.positionPubkey,
			tokenX,
			tokenY,
		});
		...
		await ctx.editMessageText(
			`${pairName ? `${escapeMarkdown(pairName)}\n` : ""}Select strategy:`,
			{
				...
				reply_markup: new InlineKeyboard()
					.text("📊 Spot", `addliq:strategy:${actionId}:spot`)
					.row()
					.text("📈 Bid-Ask", `addliq:strategy:${actionId}:bidask`)
					.row()
					.text("🔔 Curve", `addliq:strategy:${actionId}:curve`)
					.row()
		...
		setInputSession(chatId, async (text, sessionCtx) => {
			...
			if (Number.isNaN(val) || val < 0) {
				...
				if (retry >= 3) {
					await sessionCtx.reply(
						"✖ Too many invalid attempts\\. Use /addliq to retry\\.",
						MD,
					);
					return;
				}
				await sessionCtx.reply(
					"✖ Invalid amount\\. Send X amount \\(meme token, e\\.g\\. 1000\\):",
					MD,
				);
				return;
			}
			...
			setInputSession(chatId, async (text2, sessionCtx2) => {
				...
				if (Number.isNaN(val2) || val2 < 0) {
					...
					await sessionCtx2.reply(
						"✖ Invalid amount\\. Send Y amount \\(SOL/stable, e\\.g\\. 0\\.5\\):",
						MD,
					);
					return;
				}
				...
				const pairName =
					s2.tokenX && s2.tokenY ? `${s2.tokenX}/${s2.tokenY}` : "";
				const summary = [
					tgBold("Add Liquidity?"),
					pairName ? `${escapeMarkdown(pairName)}` : "",
	...
	bot.command("removeliq", async (ctx) => {
		try {
			await requireKeypair();
			const [poolAddress, positionPubkey, bps] = (ctx.match as string)
				.trim()
				.split(/\s+/);
	...
	bot.callbackQuery(/^removeliq:pool:(.+)$/, async (ctx) => {
		...
		try {
			...
			if (!detail) {
				await ctx.editMessageText("Pool not found\\.", {
					...MD,
					reply_markup: new InlineKeyboard().text("⬅️ Back", "removeliq:pools"),
				});
				return;
			}
			const kb = new InlineKeyboard();
			...
			await ctx.editMessageText(
				`*Remove Liquidity — Select Position*\nPool: ${tgCode(poolAddr)}`,
				{ ...MD, reply_markup: kb },
			);
		} catch (e) {
			await ctx.editMessageText(
	...
	bot.callbackQuery(/^removeliq:pos:(.+)$/, async (ctx) => {
		...
		await ctx.editMessageText(
			`${pairName ? `${escapeMarkdown(pairName)}\n` : ""}Select amount:`,
			{
				...MD,
				reply_markup: new InlineKeyboard()
					.text("25%", `removeliq:bps:${actionId}:2500`)
					.text("50%", `removeliq:bps:${actionId}:5000`)
					.row()
					.text("75%", `removeliq:bps:${actionId}:7500`)
					.text("100%", `removeliq:bps:${actionId}:10000`)
					.row()
					.text("✏️ Custom", `removeliq:custom:${actionId}`)
					.row()
	...
	bot.callbackQuery(/^removeliq:custom:(.+)$/, async (ctx) => {
		...
		setInputSession(chatId, async (text, sessionCtx) => {
			...
			if (Number.isNaN(bps) || bps < 1 || bps > 10000) {
				...
				if (retry >= 3) {
					await sessionCtx.reply(
						"✖ Too many invalid attempts\\. Use /removeliq to retry\\.",
						MD,
					);
					return;
				}
				await sessionCtx.reply(
					"✖ BPS must be between 1 and 10000\\. Send a number:",
					MD,
				);
				return;
			}
		...
		await ctx.editMessageText(
			"✏️ Send BPS \\(1\\-10000, e\\.g\\. 5000 \\= 50%\\):",
			MD,
		);
	});
			...
			if (poolAddress && positionPubkey) {
				...
				await present(
					...
					makeZapRunner(async (zap) => {
						...
						if (!sig)
							throw new Error("Claim produced no transaction signature");
						return sig;
					}),
				);
				return;
	...
	bot.callbackQuery(/^claimfee:pool:(.+)$/, async (ctx) => {
		...
		try {
			...
			if (!detail) {
				await ctx.editMessageText("Pool not found\\.", {
					...MD,
					reply_markup: new InlineKeyboard().text("⬅️ Back", "claimfee:pools"),
				});
				return;
			}
			const kb = new InlineKeyboard();
	...
	bot.callbackQuery(backPattern, async (ctx) => {
		...
		await ctx.editMessageText("📈 Open Positions — Select Pool", {
			...MD,
			reply_markup: kb,
		});
	});
	...
	bot.callbackQuery(/^confirm:(.+)$/, async (ctx) => {
		...
		if (!op) {
			await ctx.editMessageText(
				"⌛ Expired \\(operation no longer available\\)",
				MD,
			);
			return;
		}
	...
	async function interactivePoolList(ctx: Context, prefix: string) {
		try {
			...
			await ctx.reply("📈 Open Positions — Select Pool", {
				...MD,
				reply_markup: kb,
			});
		} catch (e) {
...
function formatOnchainCostQuote(quote: PositionCostQuote): string {
	...
	lines.push(
		`Position rent: ${escapeMarkdown(quote.positionCost.toFixed(4))} ◎ (refundable)`,
	);
	if (quote.positionReallocCost > 0) {
		lines.push(
			`Position extension: ${escapeMarkdown(quote.positionReallocCost.toFixed(4))} ◎ (refundable)`,
		);
	}
	...
	if (quote.bitmapExtensionCost > 0) {
		lines.push(
			`⚠ Bitmap extension: ${escapeMarkdown(quote.bitmapExtensionCost.toFixed(4))} ◎ (non-refundable)`,
		);
	}
