...
export function tgOpenPools(pools: readonly OpenPool[]): string {
	...
	const totalFees = pools.reduce(
		(sum, p) => sum + parseFloat(p.unclaimedFees || "0"),
		0,
	);
	...
	pools.forEach((p, i) => {
		...
		if (p.openPositionCount <= 1) {
			...
			if (live) {
				lines.push(
					`      ${escapeMarkdown(`${live.amountX} ${p.tokenX} + ${live.amountY} ${p.tokenY}`)}`,
				);
				lines.push(
		...
		for (const pos of p.listPositions) {
			...
			lines.push(
				`   ${escapeMarkdown(treeChar)} ${icon} ${tgCode(pos)}${escapeMarkdown(oorLabel)}`,
			);
			const live = p.positionsLive?.find((e) => e.address === pos);
			if (live) {
				lines.push(
...
export function tgWatchedList(wallets: readonly WatchedWallet[]): string {
	...
	for (const w of wallets) {
		...
		lines.push(`• ${tgCode(w.address)}${label}`);
	}
	lines.push("", `Total: ${escapeMarkdown(String(wallets.length))} wallets`);
	return lines.join("\n");
}
...
export function tgMultiWalletPositions(
	...
	for (const r of results) {
		const label = r.wallet.label
			...
			: "";
		lines.push(`${tgBold(`🔹 ${escapeMarkdown(r.wallet.address)}`)}${label}`);
		if (r.pools.length === 0) {
...
export function tgWatchlistAlert(
	...
	if (opts?.baseFee != null)
		poolInfo.push(`Fee: ${escapeMarkdown(opts.baseFee)}%`);
	const poolInfoStr =
	...
	if (opts?.pnl != null) {
		...
		if (opts.listPositions?.length) {
			...
			positions.forEach((pos, idx) => {
				...
				if (live) {
					lines.push(
						`     ${escapeMarkdown(`${live.amountX} ${tokenX} + ${live.amountY} ${tokenY}`)}`,
					);
					lines.push(
						`     Fees: ${escapeMarkdown(`${live.feeX} ${tokenX} + ${live.feeY} ${tokenY}`)}`,
					);
				}
			});
		} else {
		...
		if (
			opts?.prevPositionCount != null &&
			opts.prevPositionCount !== positionCount
		) {
