...
import { api, dlmm, resolveWallet, screenPools, watchlist } from "./fx.js";
...
export function registerMenu(bot: Bot) {
	...
	bot.callbackQuery(/^menu:watchlist_positions$/, async (ctx) => {
		...
		try {
			...
			await ctx.editMessageText(`✖ ${escapeMarkdown(msg)}`, {
				...MD,
				reply_markup: backKeyboard("watchlist"),
			});
		}
	});
	...
	bot.callbackQuery(/^menu:commands$/, async (ctx) => {
		...
		const text = [
			...
			"`/alerts` — show active alerts",
			"`/tpsl` — global TP/SL thresholds",
			"",
			tgBold("Agent"),
			"`/agent` — start/stop/status/portfolio/journal",
			"`/briefing` — send the daily briefing",
		].join("\n");
		await ctx.editMessageText(text, {
			...MD,
			reply_markup: backKeyboard("main"),
