...
describe("computeStrategySplit", () => {
	it("degenerate span returns 50/50", () => {
		expect(computeStrategySplit(100, 100, 100, "spot")).toEqual({
			fX: 0.5,
			fY: 0.5,
		});
	});
	it("spot is proportional to bin counts", () => {
		expect(computeStrategySplit(100, 90, 110, "spot")).toEqual({
			fX: 0.5,
			fY: 0.5,
		});
		expect(computeStrategySplit(100, 90, 130, "spot")).toEqual({
			fX: 0.75,
			fY: 0.25,
		});
	});
	it("bidask amplifies the wider side", () => {
		expect(computeStrategySplit(100, 90, 130, "bidask")).toEqual({
			fX: 0.9,
			fY: 0.1,
		});
	});
	...
	it("single-sided above yields all X", () => {
		expect(computeStrategySplit(100, 100, 130, "spot")).toEqual({
			fX: 1,
			fY: 0,
		});
	});
